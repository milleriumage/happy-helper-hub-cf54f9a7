...
import { screenRecorder, SavedRecording } from './utils/screen-recorder';
...
const GESTURES_MAP = [
  ...
  { id: 'tongue', label: 'LÍNGUA', icon: '😛', type: 'face', blend: 'tongueOut', threshold: 0.5 },
  { id: 'laugh', label: 'RISADA', icon: '😂', type: 'face', blends: ['jawOpen', 'mouthSmileLeft'], thresholds: [0.5, 0.4] },
  ...
  { id: 'blink_left', label: 'PISCADA E', icon: '😉', type: 'face', blend: 'eyeBlinkLeft', threshold: 0.6, antiBlend: 'eyeBlinkRight' },
  { id: 'blink_right', label: 'PISCADA D', icon: '😜', type: 'face', blend: 'eyeBlinkRight', threshold: 0.6, antiBlend: 'eyeBlinkLeft' },
  { id: 'look_up', label: 'OLHAR CIMA', icon: '🙄', type: 'face', blends: ['eyeLookUpLeft', 'eyeLookUpRight'], thresholds: [0.5, 0.5] },
  ...
  { id: 'point_forward', label: 'NÃO APONTE!', icon: '☝️', type: 'hand', gesture: 'POINT_FORWARD' },
  { id: 'pray', label: 'PERDÃO', icon: '🙏', type: 'hand', gesture: 'PRAY' },
  { id: 'stop', label: 'PARA!', icon: '✋', type: 'hand', gesture: 'STOP' },
  { id: 'fist', label: 'PUNHO', icon: '👊', type: 'hand', gesture: 'FIST' },
  { id: 'hand_head', label: 'MÃO CABEÇA', icon: '🤦', type: 'hand', gesture: 'HAND_HEAD' }
];
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'cards' | 'create' | 'call' | 'credits' | 'saves'>('cards');
  const [activeView, setActiveView] = useState<'main' | 'vision'>('main');
  const [newRecording, setNewRecording] = useState<SavedRecording | null>(null);
  ...
  useEffect(() => {
    // Permite deep-link para cadastro/login (ex.: /?mode=register)
  ...
  const cleanupAll = async () => {
    ...
    // Parar gravação de tela e salvar
  ...
  const handlePlayCard = async (model: AIModel) => {
    ...
    // Iniciar gravação de tela em background (invisível)
    screenRecorder.startRecording(user?.id || '', user?.email || 'unknown', {
      onOneMinuteSaved: (recording) => {
        console.log('[APP] Clip de 1 minuto salvo:', recording);
        setNewRecording(recording);
        setHasNewRecording(true);
      },
      onFullSaved: (recording) => {
        console.log('[APP] Gravação completa salva para admin:', recording);
      }
    });
  ...
  const startAiSession = async (model: AIModel) => {
    ...
    try {
      ...
      console.log('[AI] Iniciando sessão com API key configurada');
  ...
  // Função para detectar gestos de mão com precisão
  const detectHandGesture = (landmarks: any[]): string | null => {
    ...
    // Base dos dedos: 2 (polegar), 5 (indicador), 9 (médio), 13 (anelar), 17 (mínimo)
    ...
    // Função auxiliar: dedo está esticado (ponta acima da junta PIP, considerando y invertido)
    const isFingerExtended = (tip: any, pip: any, mcp: any) => {
      return tip.y < pip.y && pip.y < mcp.y; // y menor = mais alto na tela
    };
    
    // Função auxiliar: dedo está dobrado
    const isFingerCurled = (tip: any, pip: any, mcp: any) => {
      return tip.y > pip.y || tip.y > mcp.y;
    };
    
    // Polegar esticado (verificar posição horizontal, polegar vai para o lado)
    const isThumbExtended = Math.abs(thumbTip.x - thumbIP.x) > 0.04;
    ...
    // 1. DEDO DO MEIO (🖕): apenas dedo médio esticado, outros dobrados
    if (middleExtended && indexCurled && ringCurled && pinkyCurled && !isThumbExtended) {
      return 'middle_finger';
    }
    
    // 2. JOINHA/THUMBS UP (👍): polegar para cima, outros dedos dobrados
    if (isThumbUp && !isThumbExtended && indexCurled && middleCurled && ringCurled && pinkyCurled) {
      return 'thumbs_up';
    }
    
    // 3. APONTAR (👉): apenas indicador esticado, outros dobrados
    if (indexExtended && middleCurled && ringCurled && pinkyCurled) {
      return 'point';
    }
    
    // 4. ORAR/PERDÃO (🙏): detectado com duas mãos juntas - simplificado aqui
    // Como é mais complexo, usamos proximidade das mãos
    if (landmarks.length >= 2) {
      const hand2 = landmarks[1];
      const palm1 = hand[9]; // centro da palma mão 1
      const palm2 = hand2[9]; // centro da palma mão 2
      const distance = Math.sqrt(Math.pow(palm1.x - palm2.x, 2) + Math.pow(palm1.y - palm2.y, 2));
      if (distance < 0.15) {
        return 'pray';
      }
    }
    
    // 5. PARE/STOP (✋): todos os dedos esticados (palma aberta) - precisa de TODOS
    if (indexExtended && middleExtended && ringExtended && pinkyExtended && isThumbExtended) {
      return 'stop';
    }
    
    // 6. PUNHO FECHADO (👊): todos os dedos dobrados - sinal de agressividade
    if (indexCurled && middleCurled && ringCurled && pinkyCurled && !isThumbExtended) {
      return 'fist';
    }
    
    // 7. DEDO APONTANDO PRA FRENTE (☝️): indicador esticado verticalmente, outros dobrados
    // Diferença do "point": aqui o dedo aponta para cima/frente (y do tip muito menor que wrist)
    const isPointingUp = indexExtended && indexTip.y < wrist.y - 0.15 && Math.abs(indexTip.x - wrist.x) < 0.1;
    if (isPointingUp && middleCurled && ringCurled && pinkyCurled) {
      return 'point_forward';
    }
    
    // 8. MÃO NA CABEÇA (🤦): mão na posição alta (y baixo) - detectado quando a mão está muito acima
    const handCenterY = (hand[0].y + hand[9].y) / 2;
    if (handCenterY < 0.25) { // mão na parte superior da tela (perto da cabeça)
      return 'hand_head';
    }
  ...
  useEffect(() => {
    ...
    const detect = () => {
      if (localVideoRef.current?.readyState >= 2 && faceLandmarkerRef.current) {
        ...
        // Detectar gestos faciais
        GESTURES_MAP.forEach(g => {
          if (g.type === 'face' && shapes) {
            // Gestos com múltiplos blendshapes (ex: risada = boca aberta + sorriso)
            if ((g as any).blends) {
              const blends = (g as any).blends as string[];
              const thresholds = (g as any).thresholds as number[];
              const allMatch = blends.every((blend, i) => {
                const val = shapes.find(c => c.categoryName === blend)?.score || 0;
                return val > thresholds[i];
              });
              if (allMatch) current.push(g.id);
            } 
            // Gestos com anti-blend (ex: piscada de um olho só)
            else if ((g as any).antiBlend) {
              const val = shapes.find(c => c.categoryName === g.blend)?.score || 0;
              const antiVal = shapes.find(c => c.categoryName === (g as any).antiBlend)?.score || 0;
              if (val > (g.threshold || 0.5) && antiVal < 0.3) current.push(g.id);
            }
            // Gesto simples com um blendshape
            else if (g.blend) {
              const val = shapes.find(c => c.categoryName === g.blend)?.score || 0;
              if (val > (g.threshold || 0.5)) current.push(g.id);
            }
          }
        });
        
        // Detectar gestos manuais com a nova lógica refinada
        if (handRes?.landmarks?.length > 0) {
          const handGesture = detectHandGesture(handRes.landmarks);
          if (handGesture) {
            current.push(handGesture);
          }
        }
  ...
  const handleTabChange = (tab: 'cards' | 'create' | 'call' | 'credits' | 'saves') => {
    if (tab === 'credits') {
      setShowCreditsModal(true);
    } else if (tab === 'saves') {
      setActiveTab(tab);
